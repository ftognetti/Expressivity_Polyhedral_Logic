...
\begin{proposition}[Locally $\leq n$-holed $1d$]
    The class of $1d$ polyhedra such that each connected component
    has ``fewer than $n$ holes'' (i.e., $\text{rk}(H_1(X_i,\Z))\leq n$) is modally
    definable.
\end{proposition}
\begin{proof}
    Indeed, it is $\Pol(S4.\text{Grz}.\text{Bd}_n.\chi(F_{n+1}))$, where $F_k$ is the $k$-fork.
\end{proof}
