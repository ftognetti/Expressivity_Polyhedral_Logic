...
This allows us to give an inclusive definition of the resulting combinatorial object:
...
\end{tikzpicture}\]
(The example picture shows by coloring the canonical morphism $\Delta_2^{(2)}\twoheadrightarrow \Delta_2^{(1)}$: the simplices on the domain (left) are mapped to the simplex in the codomain (right) of the same colour.)
...
This suggests -but doesn't quite prove- that our notion of p-morphism, despite depending on the choice of a triangulation is actually relative to the underlying polyhedron.